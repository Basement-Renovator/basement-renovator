...
import * as fileutil from './packages/common/fileutil';
import pathlib from 'path';
import os from 'os';
import { printf, printSectionBreak, range } from 'packages/common/util';
import * as XML from 'packages/common/xml';
import type { MainLookup } from 'packages/common/lookup';
...
let settings: {
    value: <T=any>(key: string, def?: T) => T;
    setValue: <T=any>(key: string, val: T) => void;
};
/**
...
let STEAM_PATH: string | undefined;
async function getSteamPath(): Promise<string | undefined> {
    if (process.platform.includes("win")) {
        const rkey = "HKEY_CURRENT_USER\\Software\\Valve\\Steam";
        STEAM_PATH ??= (await regedit.promisified.list([rkey]))[rkey].values.SteamPath?.value as string;
        return STEAM_PATH;
    }
    else if (process.platform === "darwin") {
        return pathlib.join(os.homedir(), "Library/Application Support/Steam");
    }
    else {
        return pathlib.join(os.homedir(), ".local/share/Steam");
    }
}


async function findInstallPath(): Promise<string> {
    const version = getGameVersion();
    if (version === "Antibirth" && settings.value("AntibirthPath")) {
        return settings.value("AntibirthPath");
    }

    if ((await fileutil.stat(settings.value("InstallFolder"))).isDir()) {
        return settings.value("InstallFolder");
    }

    let cantFindPath = false;
    const basePath = await getSteamPath();
    if (!basePath) {
        printf("Could not find Steam folder");
        return '';
    }

    let installPath = pathlib.join(basePath, "steamapps", "common", "The Binding of Isaac Rebirth");

    // Windows path things
    if (process.platform === "darwin") {
        installPath = pathlib.join(installPath, "The Binding of Isaac Rebirth.app/Contents/Resources");
    }
    else if (!(await fileutil.stat(installPath)).isDir()) {
        cantFindPath = true;

        const libconfig = pathlib.join(basePath, "steamapps", "libraryfolders.vdf");
        if ((await fileutil.stat(libconfig)).isFile()) {
            const libLines = (await fileutil.read(libconfig)).split('\n');
            const matcher = /"\d+"\s*"(.*?)"/;

            const installDirs = libLines.map(line => line.match(matcher))
                .filter((res): res is Exclude<typeof res, null> => !!res)
                .map(res => pathlib.normalize(res[1]));

            for await (const root of installDirs) {
                const p = pathlib.join(root, "steamapps", "common", "The Binding of Isaac Rebirth");
                if ((await fileutil.stat(installPath)).isDir()) {
                    installPath = p;
                    cantFindPath = false;
                    break;
                }
    ...
    if (cantFindPath || !installPath || !(await fileutil.stat(installPath)).isDir()) {
...
async function findModsPath(installPath?: string): Promise<string> {
    let modsPath = "";
    let cantFindPath = false;

    if ((await fileutil.stat(settings.value("ModsFolder"))).isDir()) {
        modsPath = settings.value("ModsFolder");
    }
    else {
        installPath ??= await findInstallPath();
        if (installPath?.length > 0) {
            const modDirectoryFile = pathlib.join(installPath, "savedatapath.txt");
            if ((await fileutil.stat(modDirectoryFile)).isFile()) {
                const lines = (await fileutil.read(modDirectoryFile)).split('\n');

                const [ prefix, modDir ] = lines.map(line => line.split(": ")).find(([ part ]) => part === "Modding Data Path") ?? [];
                if (modDir) {
                    modsPath = pathlib.normalize(modDir.trim());
                }
            }
        }
    }

    if (!modsPath || !(await fileutil.stat(modsPath)).isDir()) {
        cantFindPath = true;
    }
    ...
    if (cantFindPath && version === 'Repentance') {
        cantFindPath = false;
        const installPath = await findInstallPath();
        if (installPath) {
            modsPath = pathlib.join(installPath, 'mods');
        }
        else {
            cantFindPath = true;
        }
    }
    else if (cantFindPath) {
        cantFindPath = false;
        // Windows path things
        if (process.platform.includes("win")) {
            modsPath = pathlib.join(os.homedir(), "Documents", "My Games", `Binding of Isaac ${version} Mods`);
        }
        // Mac Path things
        else if (process.platform === 'darwin') {
            modsPath = pathlib.join(os.homedir(), `Library/Application Support/Binding of Isaac ${version} Mods`);
        }
        // Linux and others
        else {
            modsPath = pathlib.join(os.homedir(), `.local/share/binding of isaac ${version.toLowerCase()} mods/`);
        }
    }

    // Fallback Resource Folder Locating
    if (cantFindPath) {
        const modsPathOut = QFileDialog.getExistingDirectory(
...
let xmlLookups: MainLookup;
function loadMods(autogenerate: boolean, installPath: string, resourcePath: string) {
    // Each mod of the mod folder is a Group
    const modsPath = await findModsPath(installPath);
    if (!(await fileutil.stat(modsPath)).isDir()) {
        printf("Could not find Mods Folder! Skipping mod content!");
        return;
    }

    const autogenPath = "resources/Entities/ModTemp";
    if (autogenerate) {
        fileutil.ensureDir(autogenPath);
    }

    printSectionBreak();
    printf("LOADING MOD CONTENT");
    for await (const mod of fileutil.dirIter(modsPath)) {
        const modPath = pathlib.join(modsPath, mod.name);
        const brPath = pathlib.join(modPath, "basementrenovator");

        // Make sure we're a mod
        if (!mod.isDir() || (await fileutil.stat(pathlib.join(modPath, "disable.it"))).isFile()) {
            continue;
        }

        // simple workaround for now
        if (!(autogenerate || (await fileutil.stat(brPath)).isDir())) {
            continue;
        }
        ...
        try {
            ...
            const metadata = await fileutil.read(pathlib.join(modPath, "metadata.xml"));
            const root = parser.decode(metadata);
            modName = root.metadata.name.innerText;
        }
        catch (e) {
...
class RoomScene extends QGraphicsScene {
    ...
    loadBackground() {
        ...
        this.floorAnim.spritesheets[0] = roomShape !== 1 && overrideBG ? overrideBG : mainBG;
...
class RoomEditorWidget extends QGraphicsView {
    ...
    paintEvent(event) {
        ...
        if (room) {
            // Room Type Icon
            roomTypes = xmlLookups.roomTypes.lookup({ room, showInMenu: true });
...
class Entity extends QGraphicsItem {
    ...
    getPitFrame(pitImg, rendered) {
        ...
        [L, R, U, D, UL, DL, UR, DR] = adjEnts.map(stack => stack.some(ent => ent.getCurrentImg() === pitImg));
        ...
        if (L && U && R && D && !DL && !DR)  F = 24;
        if (L && U && R && D && !UR && !UL)  F = 23;
        if (L && U && R && UL && !UR && !D)  F = 27;
        if (L && U && R && UR && !UL && !D)  F = 28;
        if (L && U && R && !D && !UR && !UL) F = 29;
        if (L && R && D && DL && !U && !DR)  F = 30;
        if (L && R && D && DR && !U && !DL)  F = 31;
        if (L && R && D && !U && !DL && !DR) F = 32;

        if (hasExtraFrames) {
            if (U && R && D && !L && !UR && !DR)              F = 33;
            if (U && L && D && !R && !UL && !DL)              F = 34;
            if (U && R && D && L && UL && UR && DL && !DR)    F = 37;
            if (U && R && D && L && UL && UR && DR && !DL)    F = 38;
            if (U && R && D && L && !UL && !UR && !DR && !DL) F = 39;
            if (U && R && D && L && DL && DR && !UL && !UR)   F = 40;
            if (U && R && D && L && DL && UR && !UL && !DR)   F = 41;
            if (U && R && D && L && UL && DR && !DL && !UR)   F = 42;
            if (U && R && D && L && UL && !DL && !UR && !DR)  F = 43;
            if (U && R && D && L && UR && !UL && !DL && !DR)  F = 44;
            if (U && R && D && L && DL && !UL && !UR && !DR)  F = 45;
            if (U && R && D && L && DR && !UL && !UR && !DL)  F = 46;
            if (U && R && D && L && DL && DR && !UL && !UR)   F = 47;
            if (U && R && D && L && DL && UL && !UR && !DR)   F = 48;
            if (U && R && D && L && DR && UR && !UL && !DL)   F = 49;
        }
...
namespace EntityStack {
    export class WeightSpinner extends QDoubleSpinBox {
        constructor() {
            super(this);
    ...
    export class Proxy extends QGraphicsProxyWidget {
...
class Room extends QListWidgetItem {
    ...
    constructor(
        ...
        this.gridSpawns = spawns ?? range(this.info.gridLen()).map(() => []);
    ...
    reshape(shape, doors=undefined): void {
        ...
        newGridSpawns = range(gridLen).map(() => []);
...
class HooksDialog extends QDialog {
    ...
    contentPaths() {
        return range(this.content.count()).map(i => this.content.item(i).text()) || undefined;
    }
    ...
    displayHook(newVal, oldVal) {
        if (oldVal) {
            oldVal.val = this.contentPaths();
        }
        this.setPaths(newVal.val);
    }
...
class TestConfigDialog extends QDialog {
    ...
    commands() {
        return range(this.commandList.count()).map(i => this.commandList.item(i).text()) || undefined;
    }
...
namespace FilterDialog {
    export class FilterEntry extends QWidget {
    ...
    export class TagFilterEntry extends QWidget {
    ...
    export namespace TagFilterEntry {
        export class TagFilterListItem extends QListWidgetItem {
            constructor(config) {
...
namespace StatisticsDialog {
    export class EntityStatisticsEntry {
    ...
    namespace EntityStatisticsEntry {
        export class EntityStatItem extends QTableWidgetItem {
...
class MainWindow extends QMainWindow {
    ...
    sortRoomsByKey(less) {
        ...
        rooms = range(roomList.count()).map(() => roomList.takeItem(roomList.count() - 1)).sort(less);
    ...
    getTestModPath() {
        const modFolder = findModsPath();
        const name = "basement-renovator-helper";
        return pathlib.join(modFolder, name);
    }

    makeTestMod(forceClean?: boolean) {
        const folder = this.getTestModPath();
        const roomPath = pathlib.join(folder, "resources", "rooms");
        const contentRoomPath = pathlib.join(folder, "content", "rooms");

        if ((forceClean || !mainWindow.wroteModFolder) && (await fileutil.stat(folder)).isDir()) {
            try {
                await fileutil.remove(folder);
            }
            catch (e) {
        ...
        if ((await fileutil.stat(folder)).isDir()) {
            await fileutil.remove(pathlib.join(folder, "disable.it"));

            for (const path of [roomPath, contentRoomPath]) {
                for await (const f of fileutil.dirIter(path)) {
                    if (f.isFile()) {
                        await fileutil.remove(pathlib.join(path, f.name));
                    }
                }
            }
        }
        // otherwise, make it fresh
            ...
            try {
                await fileutil.copy("./resources/modtemplate", folder);
                await fileutil.ensureDir(roomPath);
                await fileutil.ensureDir(contentRoomPath);
    ...
    writeTestData(folder, testType, floorInfo, testRooms) {
        const quot = '\\"';
        const bs = "\\";
        const strFix = x => `"${x.replace(bs, bs + bs).replace('"', quot)}"`;

        let character = undefined;
        const commands = [];
        if (settings.value("TestConfigDisabled") !== "1") {
            character = settings.value("TestCharacter");
            if (character) {
                character = strFix(character);
            }
...
\
        ...
        await fileutil.write(pathlib.join(folder, "roomTest.lua"), `\
return {
    TestType = ${strFix(testType)},
    Character = ${character ?? 'nil'}, -- Repentance only
...
\
    ...
    disableTestMod(modPath = this.getTestModPath()) {
        if ((await fileutil.stat(modPath)).isDir()) {
            await fileutil.write(pathlib.join(modPath, "disable.it"), "");
        }
    }
    ...
    findExecutablePath() {
        if (process.platform.includes("win")) {
            ...
            if (installPath) {
                let exeName = "isaac-ng.exe";
                if ((await fileutil.stat(pathlib.join(installPath, "isaac-ng-rebirth.exe"))).isFile()) {
                    exeName = "isaac-ng-rebirth.exe";
                }
                return pathlib.join(installPath, exeName);
            }
        }
