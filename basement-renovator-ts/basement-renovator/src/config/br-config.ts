        ...
        if (node) {
            ...
            if (iconSize) {
                ...
                if (parts.length === 2 && util.checkInt(parts[0]) && util.checkInt(parts[1])) {
                    this.iconSize = [ +parts[0], +parts[1] ];
                }
                else {
                    printf(`Group ${node.attrib} has invalid IconSize, must be 2 integer values`);
                }
            }
        }
    }
...
export class EntityTag {
    ...
    getTag({ node, name }: Partial<{
        ...
        if (!name) {
            name = node?.innerText;
            if (!name) {
                printf(`Attempted to get tag with no text ${node?.attrib}`);
                return undefined;
            }
        }
...
export class Version {
    ...
    constructor(node: VersionNode, versions: Record<string, Version>) {
        ...
        this.name = node.attrib.Name;
        if (!this.name) {
            printf(`Version ${node.attrib} does not have a Name`);
        }
        ...
        for (const dataNode of node.version) {
            if ("version" in dataNode) {
                const name = dataNode.version.attrib.Name;
                if (!name) {
                    printf(`Cannot load version from node ${node.attrib} without Name`);
                    this.invalid = true;
                }
                else if (!(name in versions)) {
                    printf(`Cannot load version from node ${node.attrib} because it is not defined`);
                    this.invalid = true;
                }
                else {
                ...
                addFile("entities", dataNode.entities);
            }
            else if ("stages" in dataNode) {
                addFile("stages", dataNode.stages);
            }
            else if ("roomtypes" in dataNode) {
                addFile("roomtypes", dataNode.roomtypes);
            }
            else {
