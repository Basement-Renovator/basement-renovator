...
def getEntity(id, subtype, variant):
	...
	try:
		...
		sourceImage = QImage()
		imgPath = os.path.normpath(os.path.join(isaacResourceFolder, 'gfx/', image.get('Path').replace('\\', '/')))
		...
		pixmapImg = QImage(int(frame.get('Width')), int(frame.get('Height')), QImage.Format_ARGB32)
...
def openMap():
	...
	maps = ["00.special rooms.stb", "01.basement.stb", "02.cellar.stb", "04.caves.stb", 
				"05.catacombs.stb", "07.depths.stb", "08.necropolis.stb", "10.womb.stb", 
				"11.utero.stb", "13.blue womb.stb", "14.sheol.stb", "15.cathedral.stb", 
				"16.dark room.stb", "17.chest.stb", "18.greed special.stb", "19.greed basement.stb", 
				"20.greed caves.stb", "21.greed depths.stb", "22.greed womb.stb", "23.greed sheol.stb", 
				"24.greed the shop.stb", "25.ultra greed.stb"]
	...
	for eTmp in entities:
		...
		tmp = entityRenoXML.find("entity[@ID='{0}'][@Subtype='{1}'][@Variant='{2}']".format(e['ID'], e['Subtype'], e['Variant']))
		...
		if tmp is None:
			...
			if 'pixmap' in di:
				e['pixmap'].save('resources/NewEntities/{ID}.{Variant}.{Subtype} - {Name}.png'.format(**e))
				e['pixmap'] = 'resources/Entities/{ID}.{Variant}.{Subtype} - {Name}.png'.format(**e)
				EntityOutput.add((e['ID'], e['Name'], e['Subtype'], e['Variant'], e['pixmap'], e['Group'], e['Boss'], e['Champion'], e['BaseHP']))
			else:
				EntityOutput.add((e['ID'], e['Name'], e['Subtype'], e['Variant'], '', 'Unknown'))
