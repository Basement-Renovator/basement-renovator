...
def replaceEntities(rooms, replaced, replacement):
    ...
    def fixEnt(ent, b):
        ent.Type = b[0]
        if b[1] >= 0: ent.Variant = b[1]
        if b[2] >= 0: ent.Subtype = b[2]
...
if __name__ == '__main__':
    ...
    cmdParser.setApplicationDescription('Bulk entity replacement utility script for Basement Renovator. Takes a json file path')
    cmdParser.addHelpOption()

    cmdParser.addPositionalArgument('configFile', '''json config file to grab configuration from. Format:
    {
